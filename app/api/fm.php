...
if(!empty($frRes) && $frRes != 'No Result')
{
	...
	$match_dtl = $q_match['match'];
	
	$match_result_details['id']=$match_dtl['match_id'];
	$match_result_details['status']=$match_dtl['match_status'];
	$match_result_details['isactive']=array('a'=>$match_dtl['match_player_a_isactive'],'b'=>$match_dtl['match_player_b_isactive']);
	//$result['rematch']="true";
	$result = array();
